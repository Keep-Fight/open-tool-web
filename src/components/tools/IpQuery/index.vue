  ...
  try {
    ...
    if (data.status === 'success') {
      result.value = {
        ...
        ipType: '原生 IP' // 接口通常不直接返回此字段，此处固定展示
      };
    } else {
    ...
    <div class="max-w-6xl mx-auto space-y-6">
          ...
          <div class="bg-white dark:bg-zinc-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-zinc-800 h-full">
            <div class="flex justify-between items-center mb-6">
              ...
              <button @click="fetchMyIpInfo" class="text-blue-500 text-xs flex items-center gap-1 hover:text-blue-600">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" :class="{'animate-spin': myIpInfo.status === 'checking'}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                刷新
              </button>
            </div>
            <div class="mb-6">
        ...
        <div v-if="result" class="bg-white dark:bg-zinc-900 p-6 md:p-8 rounded-2xl shadow-sm border border-slate-100 dark:border-zinc-800 relative">
          ...
          <div class="flex flex-col lg:flex-row gap-8 lg:gap-16">
            <div class="flex items-center gap-6 min-w-fit">
              <div class="w-20 h-20 bg-blue-50 dark:bg-blue-900/20 rounded-full flex items-center justify-center text-blue-600 dark:text-blue-400 font-bold text-2xl shrink-0 shadow-inner">IP</div>
              <div>
            ...
            <div class="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-4 text-sm">
              ...
              <div class="flex justify-between border-b border-slate-50 dark:border-zinc-800/50 pb-2">
                ...
                <span :class="result.isProxy === '是' ? 'text-red-500' : 'text-green-500'" class="font-bold">{{ result.isProxy }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section class="bg-white dark:bg-zinc-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-zinc-800 transition-all">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 text-center">
          ...
          >
            <div class="font-bold text-slate-800 dark:text-zinc-300 group-hover:text-blue-600 transition-colors">{{ ip.label }}</div>
